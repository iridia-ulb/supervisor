...
impl Channel {
    ...
    /// returned.
    pub async fn data(&mut self, data: &[u8]) -> Result<(), anyhow::Error> {
    ...
    async fn send_data(&mut self, ext: Option<u32>, data: &[u8]) -> Result<(), anyhow::Error> {
        let mut pos = 0;
        loop {
            debug!(
                "sending data, pos = {:?}, self.window_size = {:?}, self.max_packet_size = {:?}",
                pos, self.window_size, self.max_packet_size
            );
            let sendable = self.window_size.min(self.max_packet_size) as usize;
            if pos == 0 && data.len() - pos <= sendable {
                self.window_size -= data.len() as u32;
                self.send_data_packet(ext, CryptoVec::from_slice(data))
                    .await?;
                break;
            } else {
                let chunk = if data.len() - pos <= sendable {
                    CryptoVec::from_slice(&data[pos..])
                } else {
                    CryptoVec::from_slice(&data[pos..pos + sendable])
                };
                self.window_size -= chunk.len() as u32;
                pos += chunk.len();
                self.send_data_packet(ext, chunk).await?;
                if pos >= data.len() {
                    break;
                }
